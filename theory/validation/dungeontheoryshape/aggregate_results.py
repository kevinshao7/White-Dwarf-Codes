...
def plot_condition(rows: list[dict[str, object]], condition: int, output: Path) -> None:
    ...
    fig.suptitle(f"Condition {condition}: {condition_label(condition)}, Dungeon shape sweep")
    fig.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Aggregate Dungeon impact-parameter shape task outputs.")
    parser.add_argument("--results-dir", type=Path, default=RESULTS_DIR)
    parser.add_argument("--output-csv", type=Path, default=OUTDIR / "dungeon_impact_parameter_shape_curves.csv")
    ...
    for condition in sorted({int(row["condition"]) for row in rows}):
        output_png = OUTDIR / f"condition_{condition}_dungeon_impact_parameter_shape.png"
